...
!!! tip
    Make sure first that you are able to run your Shiny app properly using R/Rstudio to check that all required packages are installed.
...
[Download](https://github.com/appservR/appservR/releases/latest){ .md-button .md-button--primary }
...
Go to [http://localhost:8080](http://localhost:8080). You should see a running sample app (the classic Shiny's *Old Faithful Geyser Data*, with a twist: it also displays the username of whoever is logged in, see [Manage user authentication →](../recipes/manage-auth.md)). If not, see below.
...
Check our tutorials on more advanced uses of AppservR. [Recipes →](../recipes/index.md)
